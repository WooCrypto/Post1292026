...
The app blends fun & playful design for kids with discipline, education, and entrepreneur mindset development for parents. Core features include task management with AI verification, points/rewards system, streak tracking, level progression, and gift card redemptions.
...
- **Database**: Supabase (PostgreSQL) for data persistence
  ...
  - Tables: users, children, shipping_addresses, tasks, rewards, activity_log, payments, subscriptions, password_reset_tokens
  - **IMPORTANT**: Run `supabase-schema.sql` in Supabase SQL Editor to create all tables including payments and subscriptions
...
- **Child Mode**: Complete tasks, earn points, view rewards, track progress
...
- **Mail Verification System**:
  - When mail meter reaches 100%, child must verify with parent before claiming reward
  - 6-digit verification code displayed on screen, parent enters code to verify
  - Code expires after 1 hour for security
  - Verification state synced to Supabase (mail_verification_code, mail_verification_sent_at, mail_verified columns)
  - Once verified, child can proceed with reward redemption
